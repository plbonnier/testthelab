...
export default function AddEventModal({
  ...
  const handleSubmit = (event) => {
    ...
    fetch(`${import.meta.env.VITE_BACKEND_URL}/api/events`, {
      ...
      .catch((error) => {
        showNotification("Erreur lors de l'ajout de l'événement", false);
  ...
  return (
    <Modal
      ...
      className="fixed top-1/2 left-1/2 right-auto bottom-auto mr-[-50%] transform -translate-x-1/2 -translate-y-1/2 md:w-[25rem] lg:w-[40rem] text-[8px] text-center bg-[#281f31] text-white  p-2 rounded-lg font-secondary-font"
    >
      ...
      {notification.message && (
        <div
          ...
          className={`fixed bottom-3 right-4 px-2 sm:px-5 py-2 rounded-lg flex items-center ${
